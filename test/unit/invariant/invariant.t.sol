...
contract Invariant is StdInvariant, Test {
    PoolFactory factory;
    TSwapPool pool;  //(pooltoken X weth)
    ERC20Mock poolToken;
    ERC20Mock weth;
    ERC20Mock tokenB;
    ...
    function setUp() public {
        ...
        //we need to put liquidity in the pools
        poolToken.mint(address(this), uint256(STARTING_X));
        weth.mint(address(this), uint256(STARTING_Y));
        //So address(this) (the test/invariant contract)
        //now owns the tokens you want to deposit into the pool.
        poolToken.approve(address(pool), type(uint256).max);
        //since we are testing the functionality of the pool contract
        //in our fuzz test we require that the pool contract 
        //carries out the deposit transaction
        weth.approve(address(pool), type(uint256).max);
        //type(uint256).max allows the pool contract to pull any amount of 
        //weth from your address, without having to call approve again every time.
