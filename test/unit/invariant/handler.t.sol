...
contract TSwapPoolHandler is Test {
    ...
    constructor(TSwapPool _pool) {
        ...
        //getWeth function is in the tswappool contract
    ...
    function swapPoolTokenForWethBasedOnOutputWeth(uint256 outputWethAmount) public {
        if (weth.balanceOf(address(pool)) <= pool.getMinimumWethDepositAmount()) {
         ...
         //after doing some maths we get 
        // inputReserves * outputAmount = inputAmount(outputReserves - outputAmount)
        //now inputAmount = (inputReserves * outputAmount)/
        ...
        // minimumWethToDeposit makes sure that the lower limit isnt 0
    ...
    function _updateStartingDeltas(int256 wethAmount, int256 poolTokenAmount) internal {
        ...
        //what the pool currently has is the starting amount
