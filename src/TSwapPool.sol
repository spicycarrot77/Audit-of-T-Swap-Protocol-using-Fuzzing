...
contract TSwapPool is ERC20 {
    ...
    //@audit these events should be indexed as there are more 
    //than 3 parameters in each of these
    ...
    constructor(
        ...
        /**
         * In Solidity, an address is just a raw Ethereum address.
         * But to interact with a token contract (e.g., call transfer, approve, balanceOf), 
         * you need an interface that defines those functions.
         * IERC20 is that interface — it tells the compiler “this address supports ERC20 functions.”
         */
    }
    ...
    function deposit(
        ...
        uint256 maximumPoolTokensToDeposit,
        uint64 deadline
 //@audit this deadline param is not being used even thouxht it is very critical as, if someone sets a deadline, lets say the next block. After that they can still deposit    A deposit which was supposed to fail will still go through   
    )
        external
  ...
  //@audit MINIMUM_WETH_LIQUIDITY this is a constant and therefore not required to be emitted         
                wethToDeposit
            );
        }
...
// we are making an ext call in the above function then updating the state
    ...
    function _addLiquidityMintAndTransfer(
        ...
        uint256 poolTokensToDeposit,
        uint256 liquidityTokensToMint
    ) private { //its private so nobody else can call it
        _mint(msg.sender, liquidityTokensToMint);
        //  Effects
        emit LiquidityAdded(msg.sender, poolTokensToDeposit, wethToDeposit);
  //@audit event parameters are in wrong order (always a low)   

        // Interactions  EXT transactions happening at the end of the func
...
// 997/1000 = 0.997 which means 0.003 or 0.3% fee
    }
    ...
    {
        return
        // @audit should use magic numbers not literals
            ((inputReserves * outputAmount) * 10000) /
            ((outputReserves - outputAmount) * 997);
//997/10000 = 0.0997 which means 91.3% fee
// which is too much fees and not the 0.3% fee
//@audit
    }
    ...
    //@audit since this contract isn't used internally it should be
    //external to save gas 
        public
        revertIfZero(inputAmount)
        revertIfDeadlinePassed(deadline)
        returns (uint256 output)
        // @audit inputAmount param is not used anywhere
    {
    ...
    function swapExactOutput(
        ...
        uint256 outputAmount,
        uint64 deadline
    )// @audit there is no max inputAmount risk of slippage, we always need a max input amount
        public
        revertIfZero(outputAmount)
        revertIfDeadlinePassed(deadline)
        returns (uint256 inputAmount)
...
//@audit slippage protection is missing as price changes may lead to inputAmount being a very high amount
// we needed if(maxInputAmount < inputAmount) then revert
    ...
    function sellPoolTokens(
        ...
        return
        //@audit params are in wrong order 
            swapExactOutput(
                i_poolToken,
                i_wethToken,
                poolTokenAmount,
     ...
     */
    //@audit this breaks the invariant by giving the user extra tokens 
    //after 10 swaps
    function _swap(
        IERC20 inputToken,
        uint256 inputAmount,
