...
contract PoolFactory {
    ...
    // @audit info this error isn't used anywhere
    ...
    function createPool(address tokenAddress) external returns (address) {
        ...
        //q what if the name function of the ERC20 reverts
        //always be suspiscious around ERC20s
        string memory liquidityTokenName = string.concat("T-Swap ", IERC20(tokenAddress).name());
        string memory liquidityTokenSymbol = string.concat("ts", IERC20(tokenAddress).name()); //@audit .symbol should be used
        TSwapPool tPool = new TSwapPool(tokenAddress, i_wethToken, liquidityTokenName, liquidityTokenSymbol);
        //deploying of a new pool (tokenaddress X i_wethtoken)
